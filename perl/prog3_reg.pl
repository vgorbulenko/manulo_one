...
$text = "hello,hi mean some,but,def,dif. 1,2 ,3 4, , test, check, redo";

$text =~ s/(?<=,                 #проставим пробелы после ,   игнорируя цифры  <= look around
	(?!  #отрицание
		(?<=\d,) #два условия
		(?=\d)   #логическое И 
	)
...
([^=])+?=(.*)  #все символы кроме = до прервого =

[\d\D] # абсолютно любые символы
